...
datExpr <- t(p_regressed[,match(rownames(pd),colnames(p_regressed))])
...
kme=data.frame(rownames(exp), moduleColorsAutomatic, KMEs)
...
par(mar = c(6, 8.5, 3, 3))
pdf("wgcna_output/Heatmap_DatTraits.pdf",width=5,height=9)
...
#GGplotInput
MEs0$Rows=colnames(exp)
MEs0$Condition = pd$Condition
MEs0$Genotype = pd$Genotype
...
moduleOutput <- data.frame(rownames(exp))
...
TOM = TOMsimilarityFromExpr(datExpr, power= PWR,corType = "bicor",networkType="signed",TOMType="signed",TOMDenom = "mean",nThreads = 25,verbose = 5, indent = 0)
...
pdf("wgcna_output/NetworkDendrogram.pdf",width=10,height=4)
...
df$Rows=factor(df$Rows, levels = colnames(exp))
